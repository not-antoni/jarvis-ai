...
#### Additional (Optional):
- `BRAVE_API_KEY` - Enables the `jarvis search` wake phrase using Brave Search with strict safe search and aggressive Jarvis-side explicit filtering (leetspeak/spacing detection, Unicode confusable/zero-width scrubbing, punycode decoding, NSFW domain/URL blocks, and educational-context allowlists)
