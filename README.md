...
### Web search (Brave)
- `/search <query>` — returns a top-5 embed with sources.
- **Automatic prompt augmentation** — when a user's question looks time-sensitive ("latest", "today", "who won", a near-future year), Jarvis transparently fetches Brave results and injects them as an authoritative `[WEB_SEARCH]` context block. Conservative heuristic, cached, 3.5s hard timeout so it never blocks the reply.
...
- `/untimeout` · `/kick` · `/ban [delete_days:0–7]`
...
### Utility
`/jarvis` `/yt` `/news` `/remind` `/timezone` `/caption` `/gif` `/avatar` `/banner` `/clip` `/profile` `/help` `/userinfo` `/serverinfo` `/wakeword` `/blacklist` `/channel` `/features`
...
WEB_SEARCH_AUTO=true                # disable auto-augmentation (keeps /search)
...
Google's immovable **input** safety filter (not controllable via `safetySettings`). The request is failed over to another provider automatically — it's logged at `warn` level with a `[ContentBlocked]` prefix, not `error`. No action needed unless the prompt deserves to be blocked.
...
│   ├── services/
│   │   ├── handlers/      # moderation-commands, warn-commands, search-commands, ...
│   │   ├── ai/            # error-normalize, cooldown policy
│   │   ├── ai-providers*.js
│   │   ├── brave-search.js
│   │   ├── portal-auth.js
│   │   ├── portal-sessions.js
│   │   ├── jarvis-core.js
│   │   └── database.js
│   ├── server/            # Express setup, rate-limiters, health checks
...
└── tests/                 # node:test suite (96 tests and counting)
```
