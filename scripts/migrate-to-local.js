...
function serializeDoc(doc) {
    ...
    for (const [key, value] of Object.entries(doc)) {
        if (value === null || value === undefined) {
            result[key] = value;
        } else if (value._bsontype === 'ObjectId' || (typeof value.toString === 'function' && key === '_id')) {
...
async function checkMigrationStatus() {
    ...
    if (exports.length > 0) {
        ...
        if (data) {
            ...
            log.step(`Contains: ${totalDocs} documents across ${Object.keys(data).length} collections`);
        }
    }
...
`);
    ...
    if (args.includes('--restore')) {
        ...
        for (let i = 0; i < backups.length; i++) {
            const b = backups[i];
            console.log(`  ${colors.cyan}[${i + 1}]${colors.reset} ${b.name} - ${b.totalDocs || '?'} docs`);
        }
