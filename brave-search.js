...
class BraveSearch {
    ...
    extractSearchInvocation(content) {
        if (typeof content !== 'string' || !content.length) {
            return { triggered: false, query: null, explicit: false };
        }
        ...
        return { triggered, query: extractedQuery, rawQuery: rawQuerySegment, explicit: explicitDetected };
    }
    ...
    isExplicitQuery(query, options = {}) {
        ...
        if (sanitized) {
            candidates.push({ text: sanitized, allowEducationalContext: true });
        }

        if (rawSegment && (!sanitized || rawSegment !== sanitized)) {
            candidates.push({ text: rawSegment, allowEducationalContext: false });
        }
