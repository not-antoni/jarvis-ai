...
async function loadGifFrame(url) {
    ...
    try {
        ...
        // -vf "select='eq(n,30)'" selects the 31st frame specifically (User requested 30 frames wait)
        const cmd = `ffmpeg -i "${inputPath}" -vf "select='eq(n,30)'" -vframes 1 "${outputPath}" -y`;
