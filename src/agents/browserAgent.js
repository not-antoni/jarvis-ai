...
class BrowserAgent {
    ...
    get enabled() {
        return (
            this.config?.deployment?.target === 'selfhost' &&
            !!this.config?.deployment?.headlessBrowser
        );
    }
    ...
    recordOperation(success) {
        if (success) {
            ...
            if (this.consecutiveErrorCount >= this.globalErrorThreshold) {
                ...
                console.warn(
                    `[BrowserAgent] Circuit breaker opened after ${this.consecutiveErrorCount} consecutive errors`
                );
            }
        }
    }
    ...
    async ensurePuppeteer() {
        if (!this.puppeteer) {
            try {
                ...
                throw new Error(
                    'Headless browser not available. Install dependency: npm install puppeteer'
                );
            }
        }
        if (!this.browser) {
    ...
    async withRetry(fn, { maxRetries = 2, label = 'operation' } = {}) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                lastError = err;
                console.warn(
                    `[BrowserAgent] ${label} failed (attempt ${attempt + 1}/${maxRetries + 1}): ${err.message}`
                );
    ...
    async startSession(contextKey, options = {}) {
        ...
        if (this.circuitBreakerOpen) {
            throw new Error(
                'Agent circuit breaker is open - too many consecutive errors. Please try again later.'
            );
        }
        ...
        if (this.sessions.size >= this.maxConcurrentSessions) {
            throw new Error(
                `Maximum concurrent sessions (${this.maxConcurrentSessions}) reached. Please close a session or wait.`
            );
        }
        ...
        try {
            ...
            await page.setUserAgent(
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) JarvisAgent/1.0 Chrome/120 Safari/537.36'
            );
            ...
            console.log(
                `[BrowserAgent] Session started: ${contextKey} (${this.sessions.size}/${this.maxConcurrentSessions})`
            );
            return session;
        } catch (err) {
            ...
            async () => {
                ...
                try {
                    ...
                    if (this.maxDownloadBytes && len && len > this.maxDownloadBytes) {
                        const err = new Error(
                            `Download size ${(len / 1024 / 1024).toFixed(1)}MB exceeds 50MB limit`
                        );
                    ...
                    await new Promise((resolve, reject) => {
                        res.body.on('data', chunk => {
                            ...
                            if (this.maxDownloadBytes && received > this.maxDownloadBytes) {
                                ...
                                const err = new Error(
                                    `Download exceeded ${(this.maxDownloadBytes / 1024 / 1024).toFixed(0)}MB limit`
                                );
                                err.code = 'DOWNLOAD_TOO_LARGE';
                                reject(err);
                                return;
    ...
    async prune() {
        ...
        if (beforeCount !== this.sessions.size) {
            console.log(
                `[BrowserAgent] Pruned ${beforeCount - this.sessions.size} sessions, ${this.sessions.size} remaining`
            );
        }
    }
    ...
    async shutdown() {
        ...
        for (const key of sessionKeys) {
            try {
                ...
                console.warn(
                    `[BrowserAgent] Error closing session during shutdown: ${err.message}`
                );
            }
        }
