    ...
    async handleSlashCommand(interaction) {
        ...
        try {
            ...
            switch (commandName) {
                ...
                case 't': {
                    telemetryMetadata.category = 'utilities';
                    response = await slashUtility.handleT(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'yt': {
                    telemetryMetadata.category = 'search';
                    response = await slashUtility.handleYt(interaction, this.jarvis);
                    break;
                }
                case 'search': {
                    telemetryMetadata.category = 'search';
                    response = await slashUtility.handleSearch(interaction, this.jarvis);
                    break;
                }
                case 'math': {
                    telemetryMetadata.category = 'utilities';
                    response = await slashUtility.handleMath(interaction, this.jarvis);
                    break;
                }
                case 'jarvis': {
                    response = await slashUtility.handleJarvis(interaction, this.jarvis);
                    break;
                }
                case 'time': {
                    response = await slashUtility.handleTime(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'clear': {
                    response = await slashUtility.handleClear(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'help': {
                    response = await slashUtility.handleHelp(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'profile': {
                    response = await slashUtility.handleProfile(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'history': {
                    response = await slashUtility.handleHistory(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'recap': {
                    response = await slashUtility.handleRecap(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'digest': {
                    response = await slashUtility.handleDigest(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'encode': {
                    response = await slashUtility.handleEncode(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'decode': {
                    response = await slashUtility.handleDecode(interaction, this.jarvis, userId, guildId);
                    break;
                }
                case 'pwdgen': {
                    telemetryMetadata.category = 'utilities';
